  ...
  const path = {
    ...
    animateVisualElement(visualElement, target, transition, delay2, animations2) {
      ...
      if (!("x" in target || "y" in target))
        return;
      const xValue = visualElement.getValue("x", (_a = visualElement.latestValues["x"]) != null ? _a : 0);
      const yValue = visualElement.getValue("y", (_b = visualElement.latestValues["y"]) != null ? _b : 0);
      const xRaw = target.x;
      const yRaw = target.y;
      const xFrom = (_c = Array.isArray(xRaw) && xRaw[0] != null ? xRaw[0] : xValue == null ? void 0 : xValue.get()) != null ? _c : 0;
      const yFrom = (_d = Array.isArray(yRaw) && yRaw[0] != null ? yRaw[0] : yValue == null ? void 0 : yValue.get()) != null ? _d : 0;
      const xTo = Array.isArray(xRaw) ? xRaw[xRaw.length - 1] : xRaw != null ? xRaw : xFrom;
      const yTo = Array.isArray(yRaw) ? yRaw[yRaw.length - 1] : yRaw != null ? yRaw : yFrom;
...
var VisualElementDragControls = class {
  ...
  getAxisMotionValue(axis) {
    ...
    return externalMotionValue ? externalMotionValue : this.visualElement.getValue(axis, (_a = this.visualElement.latestValues[axis]) != null ? _a : 0);
  }
  snapToCursor(point2) {
    eachAxis((axis) => {
      const { drag: drag2 } = this.getProps();
